(function() {
  var AdminForm = function() {
    ...
    function select2InputGroup($el) {
      var prnt = $el.parent('.input-group');
      // console.log(prnt)
      if (prnt.length < 1) return;

      var classList = prnt.attr('class').split(/\s+/);
      $.each(classList, function(index, item) {
        if (item === 'input-group-prepend') {
          prnt.addClass('select2-bootstrap-prepend')
        } else if (item === 'input-group-append') {
          prnt.addClass('select2-bootstrap-append')
        }
      });
    }
    /**
    ...
    function processAjaxWidget($el, name) {
      ...
      var opts = {
          ...
          data: function(term, page) {
            ...
            if (filters) {
              ...
              for (var i = 0; i < filters.length; i++) {
                var name = filters[i].replace(/(^\s*)|(\s*$)/g, ""),
                  val = $('#' + name).val();
        ...
        initSelection: function(element, callback) {
          ...
          try {
            value = jQuery.parseJSON(json);
          } catch (e) {
            return ;
          }
          console.log('data-json', json, value);
          if (value) {
            if (multiple) {
              result = [];
    ...
    function processEditorWidget($el) {
      if ($el.data('editor')) {
        return ;
      }
      var lang = $el.attr('data-lang'),
        max_lines = parseInt($el.attr('data-max-lines'), 10),
        min_lines = parseInt($el.attr('data-min-lines'), 10) || 3,
        opts = {
          mode: "ace/mode/" + lang,
      ...
      $div = $('<div style="position:relative;margin:0;font-size:14px;display:inline-block;"></div>').insertBefore($el);

      var resizeEvent = function() {
        console.log('resize--', $el.parent().width());
          var $prnt =  $el.parent();
          if (!$prnt.is(':visible')) {
            return false;
          }
          $div.width($prnt.width());
          $div.height($prnt.height());
      ...
      // $el.parent('.form-group').on('show', function() {
      ...
      if (max_lines > 0) {
        opts["maxLines"] = max_lines;
      }
      if (min_lines > 0) {
        opts["minLines"] = min_lines;
      }
      editor.setOptions(opts);
      editor.setValue($el.val(), 1);
    ...
    this.applyStyle = function($el, name) {
      ...
      for (var i = 0; i < fieldConverters.length; i++) {
        var fieldConv = fieldConverters[i];
      ...
      switch (name) {
        case 'select2':
          ...
          if ($el.attr('data-custom-input') == '1') {
            $.extend(opts, {
              selectOnBlur: true,
    ...
    this.addInlineField = function(el, elID) {
      ...
      $('.form-group', $template).each(function(e) {
        var me = $(this);
      ...
      // console.log('addField => ', $el);
      $el.trigger('addField', $template);
    };
    ...
    this.applyGlobalStyles = function(parent) {
      ...
      $(':input[data-role], textarea[data-role], a[data-role]', parent).each(function() {
        var $el = $(this);
